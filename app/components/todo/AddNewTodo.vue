...
import type { IUserSpec } from '~/types/user';
...
const onSearchUser = async (search: string) => {
    ...
    const res = await $fetch<IUserSpec[]>('/api/users/specs', {
        params: {
            companyId: userStore.getCompanyId,
            search,
        },
            ...
            >
                <template #header>
                    <div class="flex items-center justify-between">
                        <h3 class="text-base font-semibold leading-6 text-gray-900 dark:text-white">Новая задача</h3>
                        <UButton
                            color="orange"
                            variant="ghost"
